...
class="product-image-home img-thumbnail px-auto" alt="Image not loaded..."/></a>`
            ...
            + `<h5 class="text-center font-weight-bold">Precio: ${product.price.toFixed(2)}</h5>`
            + '</div>'
    }
    fetch('/products/fetch/sale')
        ...
        .then((json) => {
            ...
            if (json.length === 0) {
                $('.products-data').append(`<h1 class="text-center font-weight-bold">No hay productos en la categoria: ${category}.</h1>`)
            } else {
